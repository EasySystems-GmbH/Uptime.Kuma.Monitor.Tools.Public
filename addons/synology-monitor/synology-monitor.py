...
def _fetch_public_spk_version_from_info(ref: str) -> Tuple[Optional[str], Optional[str]]:
    try:
        ...
        m = re.search(r'^version="1.5.4-0003"]+)"', data, flags=re.MULTILINE)
        if not m:
      ...
      (function () {{
        ...
        if (modeEl && nameEl) {{
          ...
          function autoName() {{
            ...
            var known = ["smart-synology-check", "storage-synology-check", "ping-synology-check", "port-synology-check", "dns-synology-check", "backup-synology-check", "synology-main"];
